...
| `pipeline-match` | Clasifica, match skills → Excel **+ Mongo tracker** (`TRACKER_DUAL_WRITE`) | `npm run agent:pipeline` |
...
Easy Apply canónico = **este repo** (GGZenLab). Applied-list = Gmail / pipeline / reconcile (hermano en `projects/qa-job-applied-list`).

Excel canónico: Escritorio (`Empleos_Tracker.xlsx`).

## Código fuente

Hoy `gmail-fetch`, `pipeline-match` y `gmail-reconcile` **delegan** al proyecto hermano `qa-job-applied-list` vía env `APPLIED_LIST_ROOT` (default: `GGZenLab-Portfolio/projects/qa-job-applied-list`).
